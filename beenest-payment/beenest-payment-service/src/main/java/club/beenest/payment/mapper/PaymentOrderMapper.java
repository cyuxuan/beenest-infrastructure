...
public interface PaymentOrderMapper {
    ...
    /**
     * 根据计划编号查询最近一条待支付订单（防重复下单）
     *
     * @param planNo 计划编号
     * @return 最近待支付订单，不存在则返回null
     */
    PaymentOrder selectLatestPendingByPlanNo(@Param("planNo") String planNo);
     ...
     * @return 最近支付订单
     */
    PaymentOrder selectLatestByPlanNo(@Param("planNo") String planNo);
    ...
    /**
     * 根据业务计划编号查询所有已成功支付的订单（含首付款+补差款）
     * 用于退款时聚合该业务单下所有可退流水，防止漏退补差单
     *
     * @param planNo 业务计划编号
     * @return 按创建时间升序排列的支付订单列表
     */
    List<PaymentOrder> selectSuccessfulByPlanNo(@Param("planNo") String planNo);
}
