...
**User authorization sync (Access Control SPI)**: Zero-coupling SPI mechanism for syncing user authorization between CAS and downstream applications. When a user is granted/revoked access to an application in CAS, the downstream system detects the change at next login/token-refresh via `CasUserAccessControlService` SPI. Downstream apps implement the SPI interface and set `cas.client.access-control.enabled=true`. Four scenarios handled: (1) has access + no local user → `createLocalUser()`, (2) no access + has local user → `disableLocalUser()`, (3) has access + has local user → `updateLocalUser()`, (4) no access + no local user → deny. CAS side increments `tokenVersion` on grant/revoke; Client Starter checks it during authentication. Fully backward compatible — not implementing the SPI leaves existing behavior unchanged. The `beenest-cas-service` subproject is the CAS overlay server implementation, containing all Java source, resources, Docker/Jib configuration.
