...
/**
 * 微信支付策略实现
 * 实现微信 App 支付和微信 JSAPI / 小程序支付功能
 * 
 * <p>支付方式：</p>
 * <ul>
 *   <li>微信 App 支付</li>
 *   <li>微信 JSAPI / 小程序支付</li>
 * </ul>
 * 
 * <h3>API文档：</h3>
 * <ul>
...
public class WechatPaymentStrategy extends AbstractPaymentStrategy {
    ...
    public boolean isEnabled() {
        return paymentConfig.getWechat().isEnabled() && (appService != null || jsapiService != null);
    }
    ...
    protected Map<String, Object> doCreatePayment(PaymentOrder paymentOrder) throws Exception {
        ...
        String paymentMethod = paymentOrder.getPaymentMethod();
        if (PaymentConstants.METHOD_WECHAT_JSAPI.equalsIgnoreCase(paymentMethod)) {
            return createJsapiPayment(paymentOrder);
        }
        return createAppPayment(paymentOrder);
    }
    ...
    protected Map<String, Object> doQueryPayment(PaymentOrder paymentOrder) throws Exception {
        ...
        boolean useJsapi = PaymentConstants.METHOD_WECHAT_JSAPI.equalsIgnoreCase(paymentOrder.getPaymentMethod());
        if (useJsapi && jsapiService == null) {
            throw new RuntimeException("微信 JSAPI 支付服务未初始化");
        }
        if (!useJsapi && appService == null) {
            throw new RuntimeException("微信 App 支付服务未初始化");
        }

        try {
            Transaction transaction = queryWechatTransaction(paymentOrder);
    ...
    public boolean cancelPayment(PaymentOrder paymentOrder) {
        ...
        boolean useJsapi = PaymentConstants.METHOD_WECHAT_JSAPI.equalsIgnoreCase(paymentOrder.getPaymentMethod());
        if (useJsapi && jsapiService == null) {
            log.error("微信 JSAPI 支付服务未初始化");
            return false;
        }
        if (!useJsapi && appService == null) {
            log.error("微信 App 支付服务未初始化");
            return false;
        }

        try {
            closeWechatOrder(paymentOrder);
     ...
     * 调用方在创建支付时，将openid存入PaymentOrder.ext.openid
     */
    private String extractOpenidFromOrder(PaymentOrder paymentOrder) {
        if (!StringUtils.hasText(paymentOrder.getExt())) {
            return null;
        }

        try {
            JsonNode extNode = objectMapper.readTree(paymentOrder.getExt());
            String extOpenid = extNode.path("openid").asText(null);
            return StringUtils.hasText(extOpenid) ? extOpenid : null;
        } catch (Exception e) {
            log.warn("解析支付订单 ext 失败 - orderNo: {}, error: {}", paymentOrder.getOrderNo(), e.getMessage());
            return null;
        }
    }

    private Map<String, Object> createAppPayment(PaymentOrder paymentOrder) {
        if (appService == null) {
            throw new RuntimeException("微信 App 支付服务未初始化");
        }
        ...
        amount.setTotal(paymentOrder.getAmount().intValue());
        amount.setCurrency(PaymentConstants.CURRENCY_CNY);
        request.setAmount(amount);
        ...
        log.info("调用微信 App 支付下单 API - orderNo: {}", paymentOrder.getOrderNo());
        ...
        paymentParams.put("partnerId", response.getPartnerId());
        paymentParams.put("prepayId", response.getPrepayId());
        ...
        paymentParams.put("sign", response.getSign());

        log.info("创建微信 App 支付订单成功 - orderNo: {}", paymentOrder.getOrderNo());
        return paymentParams;
    }

    private Map<String, Object> createJsapiPayment(PaymentOrder paymentOrder) {
        ...
        if (!StringUtils.hasText(openid)) {
            throw new RuntimeException("用户微信openid不存在，无法创建 JSAPI 支付订单");
        }
        ...
        log.info("调用微信 JSAPI 下单 API - orderNo: {}", paymentOrder.getOrderNo());
        ...
        log.info("创建微信 JSAPI 支付订单成功 - orderNo: {}", paymentOrder.getOrderNo());
        return paymentParams;
    }
    ...
    private Transaction queryWechatTransaction(PaymentOrder paymentOrder) {
        if (PaymentConstants.METHOD_WECHAT_JSAPI.equalsIgnoreCase(paymentOrder.getPaymentMethod())) {
    ...
    private void closeWechatOrder(PaymentOrder paymentOrder) {
        if (PaymentConstants.METHOD_WECHAT_JSAPI.equalsIgnoreCase(paymentOrder.getPaymentMethod())) {
        ...
        com.wechat.pay.java.service.payments.app.model.CloseOrderRequest request =
                new com.wechat.pay.java.service.payments.app.model.CloseOrderRequest();
        request.setMchid(paymentConfig.getWechat().getMchId());
        request.setOutTradeNo(paymentOrder.getOrderNo());
