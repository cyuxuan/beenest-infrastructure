...
| **用户自注册页面** | ❌ 无 | `cas-server-support-account-registration` ⛔ | 需新增（⚠️ 该模块可能不存在于 Maven Central，需实施时验证） |
| **用户自助 Profile** | ❌ 无 | `cas-server-support-account-mgmt` ⚠️ | 需新增 |
...
| **JWT 认证** | ❌ 无 | `cas-server-support-token-tickets` + `token-webflow` ⚠️ | 需新增 |
...
| **QR Code 扫码登录** | ❌ 无 | `cas-server-support-qr-authentication` ⚠️ | 需新增 |
...
| **用户自注册** | 🔶 自定义 Controller | `cas-server-support-account-registration` ⛔ | 应替换（⚠️ 需实施时验证可用性，备选：自定义 Webflow） |
...
├── cas-server-webapp-resources                   (新增) ⚠️
...
JWT 与 Token:
├── cas-server-support-token-tickets              (新增, JWT/Token 认证) ⚠️
...
扫码与无密码认证:
├── cas-server-support-qr-authentication          (新增, QR Code 扫码登录) ⚠️
...
└── cas-server-support-account-mgmt               (新增, 账户 Profile 管理) ⚠️
```
...
```groovy
// 用户注册与账户管理
// ⚠️ account-registration 在 Maven Central 上可能不存在于 7.3.6 版本
// 实施时需验证；如不可用，使用自定义 CAS Webflow 实现注册流程
implementation "org.apereo.cas:cas-server-support-account-registration" // 可能需要替代方案
// ⚠️ account-mgmt 最高稳定版为 7.2.6，无 7.3.x 稳定版
// 实施时需验证 BOM 是否管理该模块
...
> **⚠️ 模块可用性警告**：`cas-server-support-account-registration` 在 Maven Central 上可能不存在于 7.3.x 版本。
> 如果该模块不可用，备选方案为：自定义 CAS Webflow 注册页面（Thymeleaf 模板 + 自定义 RegisterAction），
> 复用下方 Groovy Provisioning 脚本的逻辑。实施时需先验证模块可用性。
...
| **用户自注册** | ⚠️ 需验证 | `cas-server-support-account-registration` | `/cas/register` | 待验证 |
...
```groovy
// build.gradle
// ⚠️ account-mgmt 最高稳定版为 7.2.6，需验证 7.3.6 BOM 是否包含
implementation "org.apereo.cas:cas-server-support-account-mgmt"
```
...
```groovy
// build.gradle
// ⚠️ 该模块可能不存在于 7.3.6，实施时需验证
// 备选方案：自定义 CAS Webflow 注册页面
implementation "org.apereo.cas:cas-server-support-account-registration"
```

> **⚠️ 可用性说明**：该模块在 Maven Central 上可能不存在于 7.3.6 版本。
> 如不可用，使用自定义 CAS Webflow 实现注册流程（Thymeleaf 模板 + 自定义 RegisterAction）。
...
| **部分模块 Maven Central 版本不匹配 7.3.6** ⚠️ | 编译失败或运行时错误 | 见下方模块可用性矩阵，每个 Phase 开始前先 `./gradlew build` 验证 |
| Groovy Provisioning 脚本出错 | 用户注册失败 | 充分测试 Groovy 脚本，添加异常处理和日志 |
...
7. **模块可用性验证**: 以下模块在 Maven Central 上的版本可能低于 7.3.6，实施前必须通过 `./gradlew build` 验证。CAS BOM 可能管理了未发布到 Central 的内部版本，或这些模块已合并到其他模块中：

| 模块 | Maven Central 最高版本 | 状态 | 备选方案 |
|---|---|---|---|
| `cas-server-support-qr-authentication` | 7.3.1 | ⚠️ 低于 7.3.6 | 自定义 QR Code Webflow |
| `cas-server-webapp-resources` | 7.3.0 | ⚠️ 低于 7.3.6 | 不引入此模块，使用默认资源 |
| `cas-server-support-token-tickets` | 7.2.0-RC5 | ⚠️ 无稳定 7.3.x | 通过 OAuth2 JWT Access Token 替代 |
| `cas-server-support-account-mgmt` | 7.2.6 (稳定) / 7.3.0-RC4 | ⚠️ 无稳定 7.3.x | 自定义 Account Profile Webflow |
| `cas-server-support-account-registration` | 不存在 | ⛔ 不存在 | 自定义注册 Webflow |
