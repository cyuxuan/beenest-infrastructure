package club.beenest.payment.wallet.service;
...
public interface IWalletService {
     ...
     * 冻结余额（可用→冻结）
     *
     * <p>将指定金额从可用余额转移到冻结余额，带乐观锁和哈希校验。</p>
     *
     * @param customerNo 用户编号
     * @param bizType    业务类型，为空时使用默认值
     * @param amountInCents 冻结金额（分），必须大于0
     * @param description  交易描述
     * @param referenceNo  关联单号
     * @return true 表示冻结成功，false 表示余额不足或乐观锁冲突
     */
    boolean freezeBalance(String customerNo, String bizType, Long amountInCents,
                          String description, String referenceNo);

    /**
     * 解冻余额（冻结→可用）
     *
     * <p>将指定金额从冻结余额转移回可用余额，带乐观锁和哈希校验。</p>
     *
     * @param customerNo 用户编号
     * @param bizType    业务类型，为空时使用默认值
     * @param amountInCents 解冻金额（分），必须大于0
     * @param description  交易描述
     * @param referenceNo  关联单号
     * @return true 表示解冻成功，false 表示乐观锁冲突
     */
    boolean unfreezeBalance(String customerNo, String bizType, Long amountInCents,
                            String description, String referenceNo);

    /**
     * 扣减冻结余额（确认扣款）
     *
     * <p>直接扣减冻结余额，不再转回可用余额。用于提现确认等场景。</p>
     *
     * @param customerNo 用户编号
     * @param bizType    业务类型，为空时使用默认值
     * @param amountInCents 扣减金额（分），必须大于0
     * @param isWithdraw 是否为提现操作，true 时更新 totalWithdraw
     * @param isConsume  是否为消费操作，true 时更新 totalConsume
     * @return true 表示扣减成功，false 表示乐观锁冲突
     */
    boolean deductFrozenBalance(String customerNo, String bizType, Long amountInCents,
