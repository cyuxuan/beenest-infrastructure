...
| **WS-Federation** | ❌ 无 | `cas-server-support-wsfederation-webflow` + `cas-server-support-ws-idp` | 需新增 |
...
| **自适应风险认证** | ❌ 无 | `cas-server-support-electrofence` | 需新增 |
...
| **用户自助 Profile** | ❌ 无 | `cas-server-support-account-mgmt` | 需新增 |
| **Swagger API 文档** | ❌ 无 | `cas-server-support-swagger` (原生) | 需新增 |
| **用户 CRUD 管理 UI** | 🔶 自定义（不完善） | 自定义 Thymeleaf 页面（CAS 内嵌） | 需完善 |
| **通知 (邮件)** | ❌ 无 | CAS 内置邮件通知 | 需新增 |
| **通知 (SMS)** | 🔶 自定义 SmsService | CAS SMS 框架 | 可保留/整合 |
| **委托认证** | ❌ 无 | `cas-server-support-pac4j-webflow` + `pac4j-oidc` / `pac4j-saml` | 需新增 |
| **监控/Metrics** | ❌ 无 | Spring Boot Actuator | 需新增 |
| **小程序认证** | ✅ 自定义 Handler | 无原生替代 | **必须保留** |
| **JWT 认证** | ❌ 无 | `cas-server-support-token-tickets` + `token-webflow` | 需新增 |
...
| **QR Code 扫码登录** | ❌ 无 | `cas-server-support-qr-authentication` | 需新增 |
| **无密码认证 (Passwordless)** | ❌ 无 | `cas-server-support-passwordless` | 需新增 |
| **认证节流/防暴力破解** | 🔶 自定义 lockout 逻辑 | `cas-server-support-throttle` + `throttle-redis` | 应替换 |
...
| **验证码 (CAPTCHA)** | ❌ 无 | `cas-server-support-captcha` | 需新增 |
| **认证中断 (Interrupt)** | ❌ 无 | `cas-server-support-interrupt-webflow` | 需新增 |
| **自适应风险 (Electrofence)** | ❌ 无 | `cas-server-support-electrofence` | 需新增（替代 adaptive-auth） |
| **WebSocket** | ❌ 无 | `cas-server-support-websockets` | 需新增（QR Code 扫码依赖） |
| **CAS 事件持久化** | ❌ 无 | `cas-server-support-events-jpa` | 需新增 |
| **用户同意 Webflow** | ❌ 无 | `cas-server-support-consent-webflow` | 需新增 |
| **使用条款 JDBC** | ❌ 无 | `cas-server-support-aup-jdbc` | 需新增 |
| **MFA 信任设备 JDBC** | ❌ 无 | `cas-server-support-trusted-mfa-jdbc` | 需新增 |
...
| `BeenestAccessStrategy` | 已继承 CAS 原生 `DefaultRegisteredServiceAccessStrategy`，需完善以替代 `cas_app_access` 表 |
...
├── cas-server-support-oauth-webflow               (新增)
├── cas-server-support-ws-idp                      (新增)
└── cas-server-support-wsfederation-webflow        (新增)
...
└── cas-server-support-pac4j-webflow             (新增, 社交登录 Webflow)
    cas-server-support-pac4j-oidc                (新增, OIDC 委托)
    cas-server-support-pac4j-saml                (新增, SAML 委托)
...
├── cas-server-support-consent-webflow            (新增, 用户同意 Webflow UI)
├── cas-server-support-aup-webflow                (新增)
├── cas-server-support-aup-jdbc                   (新增, 使用条款 JDBC 存储)
├── cas-server-support-surrogate-webflow          (新增)
├── cas-server-support-surrogate-authentication-jdbc (新增)
├── cas-server-support-electrofence               (新增, 自适应风险认证)
├── cas-server-support-interrupt-webflow          (新增, 认证中断)
...
JWT 与 Token:
├── cas-server-support-token-tickets              (新增, JWT/Token 认证)
...
扫码与无密码认证:
├── cas-server-support-qr-authentication          (新增, QR Code 扫码登录)
└── cas-server-support-passwordless-webflow       (新增, 无密码认证 Webflow)
    cas-server-support-passwordless-jpa           (新增, 无密码认证 JPA 存储)

认证安全:
├── cas-server-support-throttle                    (新增, 防暴力破解)
├── cas-server-support-throttle-redis              (新增, 节流 Redis 后端)
└── cas-server-support-captcha                     (新增, 验证码)

WebSocket:
└── cas-server-support-websockets                  (新增, QR Code 扫码/Web 推送依赖)

事件:
├── cas-server-support-events-jpa                  (新增, CAS 事件持久化)
└── cas-server-support-core-events                 (已有, 事件核心)

MFA 信任:
└── cas-server-support-trusted-mfa-jdbc            (新增, MFA 信任设备持久化)

用户注册与账户管理:
├── cas-server-support-account-registration        (新增, 用户自注册)
└── cas-server-support-account-mgmt               (新增, 账户 Profile 管理)
```
...
### Phase 1: 基础设施增强（审计 + 监控 + Admin Dashboard + 安全基础）

**目标**: 建立企业级基础设施，替换最容易被原生替代的自定义模块，增加核心安全能力。
...
CAS 原生 CAPTCHA 模块支持在登录/注册页面集成验证码，防止暴力破解和机器人攻击。
支持 Google reCAPTCHA v2/v3 和 hCaptcha。
...
> **注意**：中国大陆环境可能无法访问 Google reCAPTCHA。备选方案：
> - 使用 hCaptcha（`provider: HCAPTCHA`）
> - 自定义实现国内验证码服务（如阿里云滑块验证）通过 CAPTCHA 模块的 Groovy 扩展点

#### 1.7 认证中断 (Interrupt Webflow)

CAS Interrupt Webflow 允许在认证流程中插入中断逻辑，常用于：
- 强制密码修改（密码过期、首次登录）
- 全局公告/维护通知
- GDPR/合规同意提醒
- 按用户属性的个性化消息
...
cas:
  interrupt:
    core:
      enabled: true
      # 中断模式: AFTER / BEFORE（认证前后触发）
      trigger-mode: AFTER
    # 使用 Groovy 脚本判断是否需要中断
    groovy:
      location: classpath:interrupt.groovy
```

Groovy 脚本示例 `interrupt.groovy`：
```groovy
...
def run(Object[] args) {
    ...
    if (mustChangePassword == "true") {
        return new InterruptResponse("请修改初始密码", [
            links: ["修改密码": "/cas/account"]
        ], false, true)  // block=true, ssoEnabled=false
    }
...
CAS Events JPA 模块将认证事件（登录成功/失败、票据操作等）持久化到数据库，
提供比 Inspektr 审计更结构化的事件数据，可用于分析和报警。
...
### Phase 4.5: JWT/Token 全链路 + 扫码登录 + 无密码认证 + 认证安全 + 自适应风险

**目标**: 建立 JWT 全链路支持（资源服务认证）、QR Code 扫码登录、无密码认证、防暴力破解、自适应风险认证。
...
// QR Code 扫码登录（依赖 WebSocket 推送扫码结果）
...
#### 4.5.6 自适应风险认证 (Electrofence)

CAS Electrofence 模块提供基于风险评分的自适应认证策略。根据用户 IP 地理位置、设备指纹、
登录时间等维度计算风险分值，自动决定是否要求 MFA、阻止登录或标记可疑行为。
...
cas:
  authn:
    adaptive:
      core:
        enabled: true
        # 风险阈值：超过此值触发额外验证（0.0 - 1.0）
        risk-threshold: 0.6
      # IP 地理位置服务
      geo:
        enabled: true
        # 使用 IP 地址库（无需外部 API）
        # 可选: maxmind / googlemaps / azure / ip
        provider: ip
      # 风险评分后触发的操作
      policy:
        # 高风险：阻止登录
...
> **Electrofence vs adaptive-authentication**：CAS 7.3.x 中 `electrofence` 是官方 BOM 提供的
> 自适应风险认证模块。设计中原先引用的 `adaptive-authentication` 在官方 BOM 列表中不存在，
> 应使用 `electrofence` 替代。

#### 4.5.7 WebSocket 配置

QR Code 扫码登录依赖 WebSocket 进行实时双向通信（PC 端等待扫码结果）。
...
cas:
  websocket:
    core:
      enabled: true
      # WebSocket 端点路径
      endpoint: /cas/websocket
```

> **注意**：如果 CAS 前面有 Nginx 反向代理，需要配置 WebSocket 代理支持：
> ```nginx
> location /cas/websocket {
>     proxy_pass https://backend:8443/cas/websocket;
...
#### 5.2 BeenestAccessStrategy 完善

当前 `BeenestAccessStrategy` 已继承 CAS 原生 `DefaultRegisteredServiceAccessStrategy`。
重构方向：完善其 `authorize()` 方法，使其完全替代 `cas_app_access` 表的功能，
通过 CAS Service 定义中的属性控制实现用户-应用访问授权。

```java
// 当前已存在的类结构（无需重新创建）
public class BeenestAccessStrategy extends DefaultRegisteredServiceAccessStrategy {
    // 已继承 CAS 原生访问策略
    // 需要完善：替代 cas_app_access 表的用户-应用授权检查
    // 重构方向：从数据库查询 cas_app_access 改为基于 Service 属性配置
}
```
...
保留并完善已有的 `BeenestAccessStrategy`（已继承 `DefaultRegisteredServiceAccessStrategy`），
用于复杂业务逻辑（如时间段限制、设备限制等）：
```java
// 已存在的自定义扩展，重构方向：
// 1. 从查询 cas_app_access 表改为基于 Service 属性配置
// 2. 可组合属性检查 + REST 远程检查
public class BeenestAccessStrategy extends DefaultRegisteredServiceAccessStrategy {
    // 已继承 CAS 原生策略
}
...
| **用户自助 Profile** | ✅ Account Management | `cas-server-support-account-mgmt` | `/cas/account` | 新增 |
...
CAS 原生提供 Swagger/OpenAPI 支持（`cas-server-support-swagger`），无需引入第三方 springdoc。
...
```yaml
# application.yml — CAS 原生 Swagger 配置
cas:
  swagger:
    enabled: true
    # CAS 原生 Swagger UI 集成
```
...
| CAPTCHA 国内可访问性 | Google reCAPTCHA 不可用 | 使用 hCaptcha 或自定义国内验证码 |
...
7. **模块名称注意事项**: 以下模块名称在 CAS 7.3.6 官方 BOM 中已确认可用（基于官方集成 JAR 列表），但部分模块名与常见直觉不同：
   - `cas-server-support-throttle` + `throttle-redis`（不是 `authentication-throttle`）
   - `cas-server-support-passwordless-webflow` / `passwordless-jpa`（不是 `passwordless`）
   - `cas-server-support-token-tickets` / `token-webflow`（不是 `jwt-authentication`）
   - `cas-server-support-account-mgmt`（不是 `account-management`）
   - `cas-server-support-swagger`（CAS 原生 Swagger，无需 springdoc）
   - `cas-server-webapp-resources`（不是 `cas-server-support-webapp-resources`）
   - `cas-server-support-oauth-webflow`（不是 `cas-server-support-oauth`）
   - `cas-server-support-surrogate-webflow`（不是 `surrogate-authentication`）
   - `cas-server-support-pac4j-webflow` / `pac4j-oidc`（不是 `integration-pac4j`）
   - `cas-server-support-electrofence`（自适应风险，不是 `adaptive-authentication`）
   - `cas-server-support-captcha`（验证码）
   - `cas-server-support-interrupt-webflow`（认证中断）
   - `cas-server-support-websockets`（WebSocket 支持）
   - `cas-server-support-events-jpa`（事件持久化）
   - `cas-server-support-consent-webflow`（用户同意 Webflow）
   - `cas-server-support-aup-jdbc`（使用条款 JDBC 存储）
   - `cas-server-support-trusted-mfa-jdbc`（MFA 信任设备持久化）

### 6.3 CAS Service Management Webapp 部署说明
...
| 密码管理 | 重置/策略/历史/过期 |
| 企业级功能 | 用户同意/AUP/模拟认证/Electrofence 自适应风险/监控/CAPTCHA 验证码/认证中断 |
| 用户管理 | 自注册/自动 Provisioning/账户 Profile/会话管理/踢人下线 |
| 服务访问控制 | 属性/角色/REST 远程授权/自定义策略 |
| 认证安全 | CAPTCHA 验证码/防暴力破解(Throttle+Redis)/Electrofence 自适应风险/认证中断 |
| 自定义代码减少 | 从 ~90 个 Java 文件减少到 ~40 个 |
| CAS 原生模块数 | 从 13 个增加到 ~55 个 |
| Admin 能力 | Palantir Dashboard + Service Management + Account Management + Account Registration + Swagger + CAPTCHA + Actuator + 会话管理 |
| 事件与审计 | Inspektr 审计(JDBC) + CAS 事件持久化(JPA) + MFA 信任设备管理 |
