package club.beenest.payment.listener;
...
public class PaymentOrderExpireListener extends KeyExpirationEventMessageListener {
    ...
    private void rollbackPlanStatus(PaymentOrder order) {
        if (order == null) {
            return;
        }
        String planNo = order.getPlanNo();
        ...
        try {
            ...
            log.info("已发送订单过期取消MQ消息 - orderNo: {}, planNo: {}", order.getOrderNo(), planNo);
        } catch (Exception e) {
